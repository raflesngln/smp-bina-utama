...
public function generateKode($table,$kolom,$kd_unik)
    {
		$bulan=date('m');
        $query = $this->db->query("select MAX(RIGHT($kolom,4)) as kd_max from $table WHERE MID($kolom,4,2)='$bulan'");
        $kd = "";
        if($query->num_rows()>0)
        {
            foreach($query->result() as $k)
...
//=============== Hitung isi tabel===============================
	...
	//=============== Hitung isi tabel===============================
		function count_record($tabel,$where)
	{
