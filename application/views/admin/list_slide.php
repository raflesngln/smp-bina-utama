	...
	?>
                    ...
                    <div class="md-card md-card-hover">
                        <div class="gallery_grid_item md-card-content">
                            <a href="<?php echo base_url();?>assets/images/corousel/<?=$row->gambar;?>" data-uk-lightbox="{group:'gallery'}">
                              <img src="<?php echo base_url();?>assets/images/corousel/<?=$row->gambar;?>" alt="" class="img">
                            </a>
                            <div class="gallery_grid_image_caption">
                                <div class="gallery_grid_image_menu" data-uk-dropdown="{pos:'top-right'}">
                                    ...
                                    <div class="uk-dropdown uk-dropdown-small">
                                        <ul class="uk-nav">
                                            <li><a href="#" onclick="edit_data(<?=$row->id_slide;?>)"><i class="material-icons uk-margin-small-right">edit</i> Edit</a></li>
                                            <li><a href="#" onclick="hapus_data(<?=$row->id_slide;?>)"><i class="material-icons uk-margin-small-right"></i> Remove</a></li>
                                        </ul>
                                    </div>
                                </div>
                                <p><?php echo $row->ket_slide;?></p>
                                <span class="gallery_image_title uk-text-truncate"><?php echo $row->judul_slide;?></span>
...
<form method="post" action="javascript:void(0);" onsubmit="save_slide()" id="form_inputan" enctype="multipart/form-data" data-parsley-validate="">
...
<div class="md-card-content">
 <div class="uk-grid" data-uk-grid-margin>
          <div class="uk-width-medium-1-1">
            <label>judul_slide</label>
                            <input name="judul_slide" type="text" class="md-input label-fixed" id="judul_slide" required="required" />
          </div>
 ...
 <div class="uk-grid" data-uk-grid-margin>
          <div class="uk-width-medium-1-1">
            <label>ket_slide</label>
                            <textarea name="ket_slide" class="md-input label-fixed" id="ket_slide"></textarea>
          </div>
...
<div class="uk-grid" data-uk-grid-margin>
          <div class="uk-width-medium-1-3">
            ...
            <input type="hidden" name="id_slide2" id="id_slide2" />
	...
	var id=mydata;
           ...
           success: function(data){
			   swal.close();
			   	modal=UIkit.modal("#modal_inputan",{bgclose:false,modal:false});
				modal.show();
				$("#form_inputan").attr('onsubmit','update_slide()');
                for (var i =0; i<data.length; i++){
                   $("#id_slide").val(data[i].id_slide);
				   $("#judul_slide").val(data[i].judul_slide);
				   $("#ket_slide").val(data[i].ket_slide);
				   $("#id_slide2").val(data[i].id_slide);
...
function update_slide(){
          ...
          swal_process();
          url = '<?php echo base_url();?>admin/update_slide';
          $.ajax({
            url : url,
            type: "POST",
            ...
            success: function(data)
            {
				...
				window.location.href="<?php echo site_url()?>Admin/list_slide";
            },
...
function hapus_data(mydata){
    ...
    //Logic to delete the item
	       $.ajax({
		   type: "POST",
           url : "<?php echo site_url('admin/hapus_slide')?>",
		    data: "id="+id,
           dataType: "json",
           success: function(data){
