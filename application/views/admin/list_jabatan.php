...
.uk-tab > li.uk-active > a {
 ...
 "lengthMenu": [[10, 60, 100, -1], [10, 60, 100, "All"]],
            ...
            "ajax": {
                "url": "<?php echo site_url('Admin/listdata_jabatan')?>",
                "type": "POST",
            },
            "columns": [
...
<form method="post" action="javascript:void(0);" onsubmit="save_jabatan()" id="form_inputan" enctype="multipart/form-data" data-parsley-validate="">
...
<div class="md-card-content">
 <div class="uk-grid" data-uk-grid-margin>
          <div class="uk-width-medium-1-1">
                            ...
                            <input name="id_jabatan" type="text" class="md-input label-fixed" id="id_jabatan" required="required" />
            <span class="uk-width-medium-1-3">
                            <input type="hidden" name="id_jabatan2" id="id_jabatan2" />
          </span></div>
...
<div class="uk-grid" data-uk-grid-margin>
          <div class="uk-width-medium-1-1">
                            <label>Email</label>
                            <textarea name="nm_jabatan" class="md-input label-fixed" id="nm_jabatan"></textarea>
                        </div>
...
function new_status(){
	...
	modal.show();
	$("#form_inputan").attr('onsubmit','save_jabatan()');
...
function edit_data(mydata){
	...
	var id=mydata;
	       $.ajax({
		   type: "POST",
           url : "<?php echo site_url('Admin/load_edit_jabatan')?>",
		    data: "id="+id,
           dataType: "json",
           success: function(data){
			   swal.close();
			   	modal=UIkit.modal("#modal_inputan",{bgclose:false,modal:false});
				modal.show();
				$("#form_inputan").attr('onsubmit','update_jabatan()');
                for (var i =0; i<data.length; i++){
                   $("#id_jabatan").val(data[i].id_jabatan);
				   $("#nm_jabatan").val(data[i].nm_jabatan);
				   $("#id_jabatan2").val(data[i].id_jabatan);
...
function update_jabatan(){
          ...
          swal_process();
          url = '<?php echo base_url();?>admin/update_jabatan';
          $.ajax({
            url : url,
            type: "POST",
             data:formData,
...
function save_jabatan(){	
       ...
       // ajax adding data to database
          $.ajax({
            url : "<?php echo site_url('Admin/save_jabatan')?>",
            type: "POST",
...
function hapus_data(mydata){
    ...
    //Logic to delete the item
	       $.ajax({
		   type: "POST",
           url : "<?php echo site_url('admin/hapus_jabatan')?>",
		    data: "id="+id,
           dataType: "json",
           success: function(data){
...
function cari(){
	...
	if(txt_search==''){
		my_table.ajax.url('<?php echo site_url()?>Admin/listdata_jabatan').load();
		return false;
	} else {
		if(txt_search !=''){	
				my_table.ajax.url('<?php echo site_url()?>Admin/listdata_jabatan/'+txt_search).load();
		} else {
				my_table.ajax.url('<?php echo site_url()?>Admin/listdata_jabatan').load();		
		}
	}
